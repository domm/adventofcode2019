...
sub runit ($self, $final = 0) {
    while (1) {
        ...
        say $self->{pos}." op $op modes ".join(',',reverse @modes) if DEBUG;
        chomp($op);
        my $method = 'op_' . $op;
...
sub op_4 ($self) { # output
    my $val = $self->output($self->val);
    if ($val == 45 || $val ==75 ) {
        say $self->code->[$self->pos - 1];
        die if $val == 75;
    }
    return undef;
}
...
sub val {
    ...
    if ($pointer == 386) {
        say "pos ".($self->{pos} - 1);
        say "relpos ".$self->{relbase};
        say "at relbase ".$self->code->[$self->{relbase}];
        say "hmm, ".$caller[3] .' '.($val  || '---');

        say "at 2428:".$self->code->[2428];
        say "at 2531:".$self->code->[2531];
        say join("\n",map { $_ .': '. $self->code->[$_] } (420 .. 446));
    }
